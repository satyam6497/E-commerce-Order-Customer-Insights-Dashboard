...
pct_delivered = round(
    ((orders['order_status'] == 'delivered').sum() / len(orders)) * 100,
    2
)
...
pct_repeat_customers = round(
    repeat_customers / len(orders_per_customer) * 100,
    2
)
...
orders_per_customer_wrong = (
    Table1
    .groupby('customer_id')['order_id']
    .count()
)

repeat_customers_wrong = (
    orders_per_customer_wrong > 1
).sum()

pct_repeat_customers_wrong = round(
    repeat_customers_wrong / len(orders_per_customer_wrong) * 100,
    2
)
...
print(
    f"\nAverage Order Value: "
    f"{round(avg_order_value, 2)}"
)


print("\nOrder Value Distribution:")
print(
    order_values['total_order_value']
    .describe()
)
...
order_customer_values = pd.merge(
    order_values,
    Table1[['order_id', 'customer_unique_id']],
    on='order_id',
    how='inner'
)
...
order_customer_values['customer_type'] = (
    order_customer_values['customer_unique_id']
    .map(customer_order_counts)
    .apply(
        lambda x: 'Repeat'
        if x > 1
        else 'One-time'
...
avg_order_value_by_type = (
    order_customer_values
    .groupby('customer_type')['total_order_value']
    .mean()
)
...
print(
    avg_order_value_by_type.round(2)
)
...
print(
    ...
    f"{orders_by_state.index[0]}"
)

print(
    f"Number of orders: "
    f"{orders_by_state.iloc[0]}"
)
...
late_state_delay = (
    Table1[Table1['delivery_delay'] > 0]
